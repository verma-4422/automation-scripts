...
## creates backup of selected Directories weekly
...
function check_backup_loc {
       ...
       then
		echo "Please specify the full path of where to send the beackup to, by creating the backup_loc.conf file in the root directory"
	exit 1
	fi}
...
function perform_backup {
	...
	do
		...
		filename=$backup_pathdir_name.tar.gz
